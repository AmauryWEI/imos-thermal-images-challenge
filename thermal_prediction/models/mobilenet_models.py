...
class MobileNetV3Small_RgbMetadata(Module):
    ...
    def forward(self, image: Tensor, metadata: Tensor):
        ...
        # Network Out with MobileNet Out and metadata
...
class MobileNetV3Small_RgbMetadataMlp(Module):
    """
    Model based on a pre-trained MobileNetV3 (Small) architecture, using an RGB input image
    and its complete metadata.

    The complete MobileNetV3 model is frozen, except for the last 2 feature blocks. The
    MobileNetV3 output classifier (576 -> 1000) is removed. The metadata tensor (9) is
    first processed by a linear layer (9 -> 9), and the resulting output concatenated
    with the MobileNetV3 output (512). Finally, this tensor passes through a linear layer
    ...
    def __init__(self):
        super(MobileNetV3Small_RgbMetadataMlp, self).__init__()
        ...
        ###########################################
        # Network Output with MobileNet features + MLP
        ###########################################

        # Output of MobileNet (= 576) + Output of MLP
        self.__fc = Linear(576 + self.__mlp_out_size, 1)
    ...
    def forward(self, image: Tensor, metadata: Tensor):
        ...
        # Network Out with MobileNet Out and metadata
        out = cat((mobilenet_out, mlp_out), 1)
        # No activation function to output a continuous value
        out = self.__fc(out)
